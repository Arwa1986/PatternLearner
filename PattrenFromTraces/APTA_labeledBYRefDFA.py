...
class APTA2:
    ...
    def __init__(self, RefDFA):
        ...
        self.add_state(RefDFA.root)

        self.root = RefDFA.root
        self.alphabet = []
    ...
    def add_trace(self, trace, type):
        self.frm = 0
        currentStateRefDFA = self.RefDFA.root
        # if the graph is empty add the first trace
        if self.G.number_of_nodes() == 1:
            for i in range(len(trace)):
                nextStateLbl = self.RefDFA.get_successor(currentStateRefDFA, trace[i])
                if i == len(trace) - 1:
                    to = self.add_state(nextStateLbl, type)
                else:
                    #  add transaction
                    to = self.add_state(nextStateLbl)
                self.add_edge(self.frm, to, trace[i])
                # the distination now become the source for the next transaction
                self.frm = to
                currentStateRefDFA = nextStateLbl
        else:
            # for label in trace:
            x = 0
            i = 0
            ...
            for j in range(lable_count, len(trace)):
                nextStateID = self.RefDFA.get_successor(currentStateRefDFA, trace[j])
                if j == len(trace) - 1:
                    to = self.add_state(nextStateID, type)
                else:
                    #  add transition
                    to = self.add_state(nextStateID)
                self.add_edge(self.frm, to, trace[j])
                # the distination now become the source for the next transition
                self.frm = to
                currentStateRefDFA = nextStateID
    ...
    def get_self_loop(self):
        ...
        # while also retrieving edge data with the key "label".
